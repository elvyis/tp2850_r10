...
#if __MDINHTX_DBGPRT__ == 1
	UARTprintf("PhyAddr : %d.%d.%d.%d\n\r", HI4BIT(phyAddr[0]), LO4BIT(phyAddr[0]), HI4BIT(phyAddr[1]), LO4BIT(phyAddr[1]));
#endif
...
MDIN_ERROR_t MDINHTX_SetAudioDelay(BOOL AudioDelayOn, WORD AudioDelayMS, MDIN_ADELAY_MODE_t AudioDelayMode)
{
    ...
    if(!AudioDelayOn) { // audio delay bypass Enable
		...
		if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x1f9, StartRowAddr+96)) return MDIN_I2C_ERROR; // 500ms = 192kByte = 96row (2kByte/row)
#endif
...
static MDIN_ERROR_t MDINHTX_IRQHandler(PMDIN_HDMICTRL_INFO pCTL)
{
	...
	if ((rVal&0x01)==0) return MDIN_NO_ERROR; //no interrupt
